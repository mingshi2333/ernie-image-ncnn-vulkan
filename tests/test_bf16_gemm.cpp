...
std::string graph_for(const Shape& shape)
{
    ...
    // ParamDict distinguishes int/float from the literal; alpha/beta need a
    // decimal point because get<float>() reads the stored bits without a cast.
    graph << "7767517\n2 2\nInput input 0 1 in\n"
          << "Gemm projection 1 1 in out 0=1.0 1=1.0"
