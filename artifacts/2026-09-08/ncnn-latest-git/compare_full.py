...
for version,precision in plan['cases']:
    ...
    before=Path(plan['saved_fp32_baseline']) if precision=='fp32' else base/'full'/('baseline-'+precision)
    if version=='candidate' and (before/'native.png').exists():
        if precision=='fp32':
...
(base/'full-comparison.json').write_text(json.dumps({'scope':'Dependency A/B and unchanged historical oracle gates; no speed claim',
