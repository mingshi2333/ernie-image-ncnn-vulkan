...
prior = Path('/var/tmp/ernie-weight-cache512-v1')
...
supervisor = (prior / 'supervisor.py').read_text().replace('ernie-weight-cache512-v1-', 'ernie-runtime-mapped-cache512-v1-')
...
commands[0] += ['--model-loading', 'mapped']
...
binding_paths += [root/'outputs/runtime-model-loading-v1/compare.py', prior/'native/process.json', prior/'native/native.png']
...
plan = {
    'scope': 'Native 512x512, runtime mapped loading plus the existing 6144MiB RAM cache and 3072MiB RAM reserve; exact-output and combined-resource diagnostic with tracing, not a paired performance benchmark.',
