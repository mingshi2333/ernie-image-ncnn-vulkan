...
targets = ['ernie-image', 'ernie-model-loading-contract', 'ernie-weight-session-contract',
...
for build in ('build-dev', 'build-install-cpu'):
    for kind, command in (
        ...
        ('ctest', ['ctest', '--test-dir', build, '--output-on-failure', '-V', '-R',
                   '^model_loading_cpu$' if build == 'build-dev' and '--retry-model-loading' in sys.argv else
                   '^(pipeline_api_contract|request_validation_contract|cli_contract|model_loading_cpu|weight_session_cpu|weight_session_vulkan|weight_session_mapped_vulkan|weight_session_bf16_file_vulkan|weight_session_mapped_bf16_file_vulkan|installed_cpp_consumer)$'])
    ):
