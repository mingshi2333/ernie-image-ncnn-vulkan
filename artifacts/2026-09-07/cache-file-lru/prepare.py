...
prior = Path('/var/tmp/ernie-cache-headroom512-v1')
...
supervisor = (prior / 'supervisor.py').read_text().replace('ernie-cache-headroom512-v1-', 'ernie-cache-file-lru512-v1-')
...
binding_paths += [root/'outputs/cache-file-lru-v1/compare.py', prior/'native/process.json', prior/'native/native.png']
...
binding_paths += [base / 'build-identity.json', prior / 'comparison.json', root / 'outputs/cache-file-lru-v1/policy-basis.json']
plan = {
    'scope': 'Same native mapped512 plus 6GiB RAM cache and 3GiB reserve, with half-clean-file-LRU reclaim credit instead of inactive-only credit; all prior resource limits and native arguments unchanged. Traced regression, not formal paired performance.',
    ...
    'retention_goal': {'more_hits_than': 13, 'fewer_evictions_than': 54},
    ...
    'policy_basis': json.loads((root / 'outputs/cache-file-lru-v1/policy-basis.json').read_text()),
