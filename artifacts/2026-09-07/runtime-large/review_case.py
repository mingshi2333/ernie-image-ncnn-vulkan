...
width, height = request['runtime_size']
assert case.name == f'{width}x{height}'
selected, runtime = select_shared_instance(shared['instances'], width, height, len(fixture['ids']))
...
assert report['status'] == 'success' and report['shape'] == [width, height]
...
assert a.shape == b.shape == (height, width, 3)
delta = np.abs(a - b)
assert float(delta.mean()) == original['png']['mae'] and int(delta.max()) == original['png']['max_abs']
review = {'scope': f'Independent root re-read of complete{width}x{height} sources, inputs, execution, all25 finite tensors and PNG; no model rerun',
          ...
          'png_passed': original['png']['passed'], 'all_quality_gates_passed': bool(original['png']['passed'] and all(row['passed'] for row in rows)),
          'native_png_sha256': sha(native_png), 'official_png_sha256': sha(official_png),
          'processes': processes}
assert len(rows) == 25 and review['passed_tensors'] == original['passed_tensors']
assert review['finite_elements'] == original['total_elements'] and review['all_quality_gates_passed'] == original['passed']
for row, previous in zip(rows, original['comparisons']):
    assert row['max_abs'] == previous['max_abs_error']
    assert np.isclose(row['nrmse'], previous['nrmse'], rtol=1e-12, atol=1e-15)
review['reviewer_sha256'] = sha(Path(__file__))
with (case / 'root-review.json').open('x') as stream:
