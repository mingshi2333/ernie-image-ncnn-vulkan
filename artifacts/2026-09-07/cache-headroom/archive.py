...
dest = root / 'artifacts/2026-09-07/cache-headroom'
...
for row in checks:
    ...
    test_runs.append({'log': Path(row['log']).name, 'passed': int(count[1]),
                      'command': row['command']})
assert [row['passed'] for row in test_runs] == [10, 6, 7, 3]
...
for name in ('prepare.py', 'compare.py', 'check.py', 'archive.py', 'checks.json', 'verification.json'):
