...
selected, runtime = select_shared_instance(shared['instances'], 768, 768, len(fixture['ids']))
...
assert report['status'] == 'success' and report['shape'] == [768, 768]
...
assert a.shape == b.shape == (768, 768, 3)
delta = np.abs(a - b)
assert float(delta.mean()) == original['png']['mae'] and int(delta.max()) == original['png']['max_abs']
review = {'scope': 'Independent root re-read of complete768 sources, inputs, execution, all25 finite tensors and PNG; no model rerun',
          ...
          'processes': processes}
assert len(rows) == 25 and review['passed_tensors'] == 18 and review['finite_elements'] == 13829120
with (base / 'review768.json').open('x') as stream:
