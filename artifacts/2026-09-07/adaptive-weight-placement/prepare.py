"""Freeze one adaptive-RAM runtime regression without GPU stress allocations."""
import hashlib
import json
...
prior = Path('/var/tmp/ernie-runtime512-mapped-v1')
...
supervisor = (prior / 'supervisor.py').read_text().replace('ernie-runtime512-mapped-v1-', 'ernie-weight-placement512-v1-')
...
commands[0] += ['--dit-weights', 'auto', '--gpu-reserve-mib', '8192', '--gpu', '0']
...
plan = {
    'scope': 'Native 512x512, automatic placement with a deliberately large 8192MiB reserve to select RAM from live driver budget; not GPU-exhaustion recovery or a performance benchmark.',
    ...
    'expected_weight_requests': {'host': 304, 'device': 0, 'reason': 'budget'},
