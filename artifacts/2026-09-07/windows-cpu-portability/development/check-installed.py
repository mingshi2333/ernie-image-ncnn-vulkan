...
OUT = Path("/var/tmp/ernie-windows-sdk-v4")
...
try:
    ...
    wrapper = ["bwrap", "--die-with-parent", "--unshare-net", "--ro-bind", "/", "/",
               "--bind", str(OUT), str(OUT), "--bind", env["TMPDIR"], "/tmp",
               "--tmpfs", str(REPOSITORY),
    ...
    unknown = "--未知 🖼"
    message = run(["/usr/bin/wine", executable, unknown, "value"],
                  "installed-utf8-argument", wrapper, expected=1)
    if unknown not in message:
        raise RuntimeError("Windows command line lost Unicode characters")
    prompt_path = OUT / "提示词 🍎.txt"
    prompt_path.write_bytes(bytes([255]))
    message = run(["/usr/bin/wine", executable, "--model", damaged,
                   "--prompt-file", prompt_path, "--output", OUT / "图像 🖼.png"],
                  "installed-utf8-prompt-path", wrapper, expected=1)
    if "UTF-8" not in message:
        raise RuntimeError("Unicode prompt path did not reach content validation")
    ...
    run(["/usr/bin/wineserver", "-k"], "wine-prefix-stop", wrapper)
    run(["/usr/bin/wineserver", "-w"], "wine-prefix-wait", wrapper)
