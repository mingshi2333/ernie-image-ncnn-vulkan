...
if side not in ('on','off'):raise SystemExit('usage: supervisor.py on|off')
p=json.loads((b/'plan.json').read_text());g=p['guard'];out=b/side
uid=os.getuid();unit='ernie-o1-metrics64-'+side+'-'+uuid.uuid4().hex
...
cmd=['systemd-run','--user','--scope','--quiet','--unit='+unit,f"--property=MemoryMax={g['memory_max_bytes']}",f"--property=MemorySwapMax={g['memory_swap_max_bytes']}",'taskset','-c',','.join(map(str,g['cpu_affinity'])),sys.executable,str(b/'worker.py'),side]
def available():
...
with (out/'supervisor.log').open('wb') as log,(out/'samples.jsonl').open('w') as samples:
 ...
 try:
  while proc.poll() is None:
   ...
   if cg.exists():
    state['cgroup_seen']=True;state['memory_max_observed']=(cg/'memory.max').read_text().strip();state['memory_swap_max_observed']=(cg/'memory.swap.max').read_text().strip();state['peak_memory_current']=max(state['peak_memory_current'],int((cg/'memory.current').read_text()));state['memory_events']=(cg/'memory.events').read_text()
