...
def reference(package,prompt,output,steps,device='cpu',initial_path=None):
    ...
    meta=real_reference([package/f'text/block-{i:02d}' for i in range(25)],prompt,text_dir,
                        source_weights['text'] if source_weights else None,cfg['text_bucket'])
    ...
    for i,t in enumerate(scheduler.timesteps):
        ...
        for index in range(36):
            ...
            expected_hash=(source_weights['dit'][index] if source_weights else
                json.loads((package/f'dit/block-{index:02d}/model.json').read_text())['weights_sha256'])
...
def main():
    ...
    if not args.reference and not args.reference_only:
        # CUDA keeps its context alive after empty_cache(). A separate reference
        ...
        if process.returncode:
            raise RuntimeError('Official reference failed; inspect reference.log and oracle/ before retrying')
        args.reference=oracle/'reference'
    if args.reference:
    ...
    result['resources']={'scope':'One native run including SHA256 verification and trace; not a controlled performance comparison'}
    for key,pattern in [('max_rss_kib',r'Maximum resident set size \(kbytes\):\s*(\d+)'),
                        ('total_seconds',r'Total: ([\d.]+) s'),
                        ('model_verification_seconds',r'Model verified: ([\d.]+) s'),
                        ('vae_and_png_seconds',r'VAE and PNG: ([\d.]+) s')]:
        match=re.search(pattern,log)
        if match:result['resources'][key]=int(match[1]) if key=='max_rss_kib' else float(match[1])
