...
def parse_map(text):
    ...
    for line in text[positions[3]:positions[4]].splitlines():
        output_name = re.match(r'^(\.[^\s]+)(?:\s|$)', line)
        if output_name:
            output_section = output_name[1]
        match = expression.match(line)
        if match:
            name, address, size, origin = match.groups()
            assert re.fullmatch(r'[^\s]+\.a\([^)]+\)|[^\s]+\.o', origin), origin
            rows.append({'section': name or pending, 'output_section': output_section,
                         'address': int(address, 16),
                         'size': int(size, 16), 'origin': origin})
            assert rows[-1]['section'] and output_section, 'Unidentified input/output section'
            pending = None
        else:
...
def main():
    ...
    alloc_by_name = {section['name']: section for section in alloc}
    assert len(alloc_by_name) == len(alloc)
    ...
    for row in rows:
        ...
        total['output_sections'].add(row['output_section'])
        # TLS NOBITS VMA may overlap ordinary output sections. Associate by
        # the map's output-section header, never by address alone.
        section = alloc_by_name.get(row['output_section'])
    ...
    for total in source_totals.values():
        section_names = sorted(total.pop('input_sections'))
        total['unique_input_section_names'] = len(section_names)
        total['input_section_names_sha256'] = hashlib.sha256(
            json.dumps(section_names, separators=(',', ':')).encode()).hexdigest()
        total['output_sections'] = sorted(total['output_sections'])
    ...
    payload = {
        ...
        'limitations': [
            ...
            'SHF_ALLOC intersections use matching map output-section names to avoid overlapping TLS VMA double counts; they do not prove symbol-level ownership or license obligations.',
    ...
    destination = output / 'analysis-v2.json'
