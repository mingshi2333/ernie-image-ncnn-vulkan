...
int main(int argc,char**argv){
 if(argc!=5){std::cerr<<"MODEL FIXTURE NEW_OUTPUT BLOB\n";return 2;}
 int status=0;
 try{
  ...
  if(name!="75"&&name!="73"&&name!="70"&&name!="88"&&name!="87"&&name!="81")throw std::runtime_error("Unreviewed diagnostic boundary");
  ...
  ncnn::VkMat boundary,final;
  {ncnn::VkCompute cmd(device);check(extractor.extract(name.c_str(),boundary,cmd));check(extractor.extract("out0",final,cmd));check(cmd.submit_and_wait());}
  ncnn::Mat hb,hf;ncnn::Option plain=option;plain.use_packing_layout=false;
  {ncnn::VkCompute cmd(device);cmd.record_download(boundary,hb,plain);cmd.record_download(final,hf,plain);check(cmd.submit_and_wait());}
  save(output/"boundary.f32",hb);save(output/"out0.f32",hf);
