...
#include "diagnose_splitk_layer.h"
#include "gpu.h"
...
int main(int argc,char**argv){
 if(argc!=7){std::cerr<<"MODEL FIXTURE NEW_OUTPUT BLOB SHADER_DIR SCREEN_WEIGHT_KN\n";return 2;}
 int status=0;
 try{
  q2_shader_directory=argv[5];q2_screen_weight=argv[6];
  fs::path model=argv[1],fixture=argv[2],output=argv[3];std::string name=argv[4];
  if(fs::exists(output))throw std::runtime_error("Output exists");
  if(name!="75"&&name!="73"&&name!="70"&&name!="88"&&name!="87")throw std::runtime_error("Unreviewed diagnostic boundary");
  ...
  ncnn::Net net;net.opt=option;net.set_vulkan_device(device);check(ernie::register_layers(net));check(net.register_custom_layer("Q2SplitDown",q2_create));
  ...
  ncnn::VkMat boundary,final,upstream;
  {ncnn::VkCompute cmd(device);check(extractor.extract("87",upstream,cmd));check(extractor.extract(name.c_str(),boundary,cmd));check(extractor.extract("out0",final,cmd));check(cmd.submit_and_wait());}
  ncnn::Mat hb,hf,hu;ncnn::Option plain=option;plain.use_packing_layout=false;
  {ncnn::VkCompute cmd(device);cmd.record_download(upstream,hu,plain);cmd.record_download(boundary,hb,plain);cmd.record_download(final,hf,plain);check(cmd.submit_and_wait());}
  save(output/"upstream87.f32",hu);save(output/"boundary.f32",hb);save(output/"out0.f32",hf);
  std::cout<<"{\"blob\":\""<<name<<"\",\"boundary_whdc\":["<<hb.w<<","<<hb.h<<","<<hb.d<<","<<hb.c<<"],\"final_whdc\":["<<hf.w<<","<<hf.h<<","<<hf.d<<","<<hf.c<<"]}\n";
