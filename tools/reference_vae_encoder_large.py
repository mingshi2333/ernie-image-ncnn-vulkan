...
def run_reference(output,w,h,rgb,scope):
 ...
 direct_url_path=Path(distribution.locate_file('diffusers-'+distribution.version+'.dist-info/direct_url.json'))
 fixture={'component':'vae-encoder','width':w,'height':h,'text_tokens':0,'scope':scope,
  ...
  'diffusers_installed':{'version':distribution.version,'package_root':str(Path(inspect.getfile(AutoencoderKLFlux2)).parents[2]),
                         'direct_url':json.loads(direct_url_path.read_text()) if direct_url_path.exists() else None},
  'vae_config_sha256':sha256(config_path),
  ...
  'wrapper_comparison_status':'not_run','boundaries':['mean','packed','normalized'],'inputs':{'in0':tensor(output/'in0.f32',inp)},
