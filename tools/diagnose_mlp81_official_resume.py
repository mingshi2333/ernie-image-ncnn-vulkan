...
def run(plan_path):
 ...
 result={'scope':'Matched-input MLP diagnostic; not official acceptance','status':'completed','plan_sha256':sha(plan_path),'baseline_sha256':hashes,'native81_sha256':native81,'tail_sha256':tail_hashes,'forwards':{'native_full_block':0,'official_full_block':1,'official_mlp_only':int(needed)},'baseline_hook_counts':counts,'tail_hook_counts':tail_counts,'tail_skipped_81_exact':not needed,'shapes':SHAPES,'formal_acceptance':False,'native_replay_reused':True}
