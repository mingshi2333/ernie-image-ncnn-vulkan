...
def pid_values(pid):
 ...
 mapped=sorted({line.split()[-1] for line in Path(f'/proc/{pid}/maps').read_text().splitlines() if len(line.split())>=6 and line.split()[-1].startswith('/') and '.so' in line.split()[-1] and Path(line.split()[-1]).is_file()})
 return {'mapped_library_paths':mapped,'pid':pid,'name':status.get('Name'),'rss_bytes':int(status.get('VmRSS','0 kB').split()[0])*1024,'vm_swap_bytes':int(status.get('VmSwap','0 kB').split()[0])*1024,'cgroup':group_for(pid),'affinity':sorted(os.sched_getaffinity(pid))}
...
def run(plan_path):
 ...
 runtime_seen={};begin=time.monotonic();initial_events=events(first['cgroup_values']);reason=None;child=None;peak=0;gpupeak=0
 ...
 with (out/'execution/worker.log').open('w') as log,(out/'execution/samples.jsonl').open('w') as samples:
  try:
   child=subprocess.Popen([sys.executable,str(out/'execution/diagnose_up84_sequence.py'),str(plan_path)],stdout=log,stderr=subprocess.STDOUT,env=env,start_new_session=True)
   while True:
    ...
    for item in observation['pids']:
     for file in item['mapped_library_paths']:
      resolved=str(Path(file).resolve())
      if resolved not in runtime_seen:
       require(resolved in p['bound'],'Unbound mapped runtime '+resolved)
       require(sha(resolved)==p['bound'][resolved],'Changed mapped runtime '+resolved)
       runtime_seen[resolved]=p['bound'][resolved]
 ...
 (out/'execution/observed-runtime.json').write_text(json.dumps(runtime_seen,indent=2)+'\n')
 result={'status':'completed' if rc==0 and reason is None else 'guard_or_child_failure','return_code':rc,'stop_reason':reason,'elapsed_seconds':time.monotonic()-begin,'peak_rss_sum_bytes':peak,'peak_whole_gpu_mib':gpupeak,'scope':group,'memory_max_bytes':MEMORY_MAX,'rss_limit_bytes':RSS_MAX,'swap_max_bytes':0,'cpu_quota_percent':200,'cpu_affinity':[0,2],'minimum_host_available_bytes':HOST_MIN,'gpu_limit_mib':GPU_MAX,'native_forwards_this_attempt_max':1,'formal_acceptance':False}
