...
**安装版新增实测：512×512 完整离线出图通过。** 隐藏源码、禁网后，安装版使用同一个共享包完成原生文本编码、8 步 Vulkan FP32 与 CPU VAE，25/25 张量通过原数值检查，PNG 最大通道差为 1/255。CPU/Vulkan 两个 SDK 的迁移安装和外部 C++ 调用也通过。新的 64×64 灰图已与官方对照：两端均为灰色纹理，25/25 数值检查通过，不能把该灰图归因于移植错误。见[实际图像、SDK 修复与完整记录](artifacts/2026-09-07/runtime-images-and-sdk/README.md)。
