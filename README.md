...
**2026-09-07：权重映射加载已接入，默认关闭。** 固定 64×64 完整生成的 27 个轨迹文件及 PNG 逐字节保持一致，该次诊断计时从 373.09 秒降至 275.76 秒。它触及 10 GiB 进程组内存上限，尚不是正式速度或内存优势结论，见[实际记录与限制](artifacts/2026-09-07/mapped-model-loading/README.md)。固定 1376×768 的 CPU 解码器和两个 head 也已通过完整元素对照，见[组件记录](artifacts/2026-09-07/fixed1376-cpu-components/README.md)；[36 个 DiT block 的连续轨迹](artifacts/2026-09-07/fixed1376-block-chain/README.md)现已通过全部 36 个输出检查，新尺寸的 8 步完整图像仍待验收。中文及长提示词的历史质量失败、与参考项目的正式配对比较及其他平台验收仍未完成，当前不能宣称全面超过参考项目。
...
输出路径必须不存在，父目录可自动创建。分辨率由转换时的静态模型桶确定；已有 64×64、1024×1024 和 512×384 包。文本编码桶为 **32、64 或 2048 tokens（含 BOS）**，上述 1024 包为 64。`--width` 与 `--height` 必须一起给出并与包一致；新尺寸/容量由 `tools/prepare_variant.py` 独立导出和验证。超出所选桶的提示词拒绝，官方分词器的 2048-token 总上限和截断约定保留。CPU 运行需同时指定 `--device cpu --precision fp32`。
