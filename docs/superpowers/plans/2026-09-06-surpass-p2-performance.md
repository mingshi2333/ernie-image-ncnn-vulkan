...
## 2026-09-07 普通读取临时数组候选

检查加载路径发现固定 ncnn 将 FP16/BF16 对齐字节数作为 16 位元素数分配，临时数组为所需的两倍。构建目录内两行修正及真实 ModelBin 分配/解码测试已实现，默认关闭；CPU/Vulkan 构建在 OFF/ON 下各 3 项 CPU 检查通过。见 [候选证据](../../../artifacts/2026-09-07/compact-model-reader/README.md)。当前只证明该临时数组减半，没有整机内存或速度收益；候选完整模型验证安排在现有固定比较结束后。现有比较仍使用原始读取器，不能拿来证明本修正有效，O2 和正式 S/M 保持未完成。
