<!-- Draft for Tencent/ncnn Discussions, Show and tell. Prepared 2026-09-08.
The repository is currently private. Publishing this post and changing repository
visibility require separate authorization. Source links use the CI branch.
-->

# ERNIE-Image-Turbo 的 ncnn/Vulkan 移植记录：从一个 DiT block 到本地出图

草稿状态：项目仓库目前为私有，本文的项目源码和实验记录链接需要仓库访问权限。发布到 ncnn Discussions 前还需准备可公开访问的对应材料。

这个项目把 ERNIE-Image-Turbo 的文本编码、DiT 去噪和 VAE 解码接成了原生程序。准备好模型包后，可以在本地离线生成图片，运行时不需要 Python 或 PyTorch。默认使用 CPU 文本编码、Vulkan DiT 和 CPU VAE，也提供公共 C++ 接口。

这篇记录面向会 C++、了解 Transformer 的读者，按组件转换、原生执行、数值定位和平台验证的顺序说明实现过程。示例命令以 Linux 上的转换环境为准；转换完成的权重可以用于其他平台的原生程序，是否通过该平台验证仍需单独检查。

当前实图证据主要来自 RTX 4060 Laptop 8GB、32GB RAM。1024×1024 和多种更大尺寸已经完成官方对照，但部分长提示词、中文和低精度样例仍有未通过项。项目定位是技术预览版。

项目：[ernie-image-ncnn-vulkan](https://github.com/mingshi2333/ernie-image-ncnn-vulkan/tree/codex/surpass-reference)。模型来自 [ERNIE-Image](https://github.com/baidu/ERNIE-Image/tree/1f30f5bb499726c606da0a24cc2a4a6c4d6ad775) 和 [ERNIE-Image-Turbo](https://huggingface.co/baidu/ERNIE-Image-Turbo/tree/bc68c81e2a1730a394d5fc9fae70713dee940140)。组织方式参考了 [zimage-ncnn-vulkan](https://github.com/nihui/zimage-ncnn-vulkan/tree/c1938eef9cd7e03cb216da4f4c89a0c041543e7a) 和 [futz12 的 ERNIE 移植](https://github.com/futz12/ernie-image-ncnn-vulkan/tree/8dcd6e4411137d8abe92c9d78581c4c96d5182c6)。本文没有同机、同配置的移植项目速度排名。
...
| 依赖 | 本项目使用的版本或提交 | 作用 |
|---|---|---|
| ncnn | `6a1bf000f363714839a36793addc8c879d3d899e` | CPU/Vulkan 原生执行 |
